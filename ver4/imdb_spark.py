...
def main():
    ...
    path=output_folder+"task1"
    ukrainian_series_movies_titles.write.csv(path, header=True, mode='overwrite')
    #ukrainian_series_movies_titles.write.format("csv").mode("overwrite").save(path)
    ...
    path=output_folder+"task2"
    people_born_in_19th_century.write.csv(path, header=True, mode='overwrite')
    ...
    path=output_folder+"task3"
    movies_last_more_2_hours.write.csv(path, header=True, mode='overwrite')
    ...
    path=output_folder+"task4"
    people_and_characters_played.write.csv(path, header=True, mode='overwrite')
    ...
    path=output_folder+"task5"
    top_100_adult_movies_per_region.write.csv(path, header=True, mode='overwrite')
    ...
    path=output_folder+"task6"
    top_50_tvseries_by_episodes.write.csv(path, header=True, mode='overwrite')
    ...
    path=output_folder+"task7"
    ten_most_popular_titles_by_each_decade.write.csv(path, header=True, mode='overwrite')
    ...
    path=output_folder+"task8"
    ten_most_popular_titles_by_each_genre.write.csv(path, header=True, mode='overwrite')
    
    # Loading
    #for task_num in range(0, len(tasks)):
        # write results to task1, task2, ... taskN .csv files
        #path=f"{output_folder}task{task_num + 1}"
        #tasks[task_num].write.csv(f"{output_folder}task{task_num + 1}", header=True, mode='overwrite')
