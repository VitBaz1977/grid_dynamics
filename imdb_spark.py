...
def main():
    ...
    for task_num in range(0, len(tasks)):
        ...
        path=f"{output_folder}\\task{task_num + 1}"
        fname=f"task{task_num+1}.csv"
        full_name=path+"\\"+fname
        saver.save_to_csv(full_name,tasks[task_num])
       # tasks[task_num].write.format("csv").mode("overwrite").save(f"{output_folder}task{task_num + 1}")
        #tasks[task_num].write.format("csv").mode("overwrite").save(full_name)
